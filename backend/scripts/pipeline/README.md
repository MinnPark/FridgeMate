...
> 주의 - 서빙 vs 정제본 카운트가 다른 이유: 현행 서빙 `recipe_db`(backend/data/chroma)는 **1,693건**(cookrcp 1146 + mafra 537 + seed 10, 드롭 전 구 파싱본)이다. 이 파이프라인 정제본 **1,689건**은 재료 0개 4건을 뺀 신 코퍼스로, s4 `--apply` 로 서빙 DB를 교체할 때 반영된다. 즉 1,693=현행 서빙, 1,689=신 정제본(미적용). 다른 문서에서 1,693이 보이면 현행 서빙 DB 기준이다.
